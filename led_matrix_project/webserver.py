...
def create_html(effect_name, animations, files, tz_value=0, manual_dt=None,
    ...
    # DEFCON threat panel (IDIOT Monitor)
    P.append('<section class="card threat">'
             '<div class="card-head">%s IDIOT Monitor <span class="threat-modes" id="threatMode">--</span></div>'
...
function updateThreats(){fetch('/threats').then(function(r){return r.json()}).then(function(t){var b=document.getElementById('defconBadge');var l=document.getElementById('defconLabel');var m=document.getElementById('threatMode');var d=document.getElementById('threatDetails');if(!b)return;var lv=t.defcon||5;b.className='defcon-badge lv'+lv;b.textContent='DC '+lv;var labels=['','CRITICAL - under attack','HIGH - attack suspected','ELEVATED','GUARDED','All clear'];l.textContent=labels[lv]||'Unknown';m.textContent=(t.mode||'none')+(t.telegram_enabled?' \u00B7 TG':'');if(t.mode!=='frames'){d.innerHTML='<div class="mac-note">Frame-level detection needs compiled firmware. Using connection monitor.</div>';return}var html='';if(t.attack_type){html+='<div class="mac-row"><span class="mac-label">Attack type</span><span class="mac-val">'+t.attack_type+(t.attack_subtype?' / '+t.attack_subtype:'')+'</span></div>'}function mac(mac,ssid){return '<div class="mac-row"><span class="mac-label"></span><span class="mac-val">'+mac+(ssid?' <span class="mac-note">-> '+ssid+'</span>':'')+'</span></div>'}if(t.target_bssid)html+='<div class="mac-row"><span class="mac-label">Target AP</span><span class="mac-val">'+t.target_bssid+(t.target_ssid?' <span class="mac-note">-> '+t.target_ssid+'</span>':'')+'</span></div>';if(t.last_src)html+='<div class="mac-row"><span class="mac-label">Source (spoofed?)</span><span class="mac-val">'+t.last_src+(t.last_src_ssid?' <span class="mac-note">-> '+t.last_src_ssid+'</span>':'')+'</span></div>';if(t.victim)html+='<div class="mac-row"><span class="mac-label">Victim</span><span class="mac-val">'+t.victim+(t.victim_ssid?' <span class="mac-note">-> '+t.victim_ssid+'</span>':'')+'</span></div>';if(t.recent_sources&&t.recent_sources.length){var parts=[];for(var i=0;i<t.recent_sources.length;i++){var s=t.recent_sources_ssids&&t.recent_sources_ssids[i];parts.push(t.recent_sources[i]+(s?'->'+s:''))}html+='<div class="mac-row"><span class="mac-label">MACs seen ('+t.total_deauths+' frames)</span><span class="mac-val">'+parts.join('  ')+'</span></div>'}if(!html)html=t.under_attack?'<div class="mac-note">Attack in progress, capturing...</div>':'<div class="mac-note">No deauth frames detected.</div>';else if(t.under_attack)html+='<div class="mac-note">Source is usually the spoofed AP BSSID, not the real attacker.</div>';d.innerHTML=html}).catch(function(){})}
