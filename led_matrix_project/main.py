...
from effects import (MatrixRain, GameOfLife, Marquee, SpriteAnimation, DigitalClock,
                     ...
                     DefconEffect)
from webserver import WebServer, create_html, create_config_html
from wifimon import WifiMonitor
...
def connect_wifi(ssid, password):
    """Connect to WiFi. If no SSID configured or connection fails, start a
    setup Access Point. Returns (ip, is_ap_mode, is_setup_mode)."""
    global setup_mode, _wlan_sta
    setup_mode = False
    ...
    # No config or connection failed: start setup Access Point
    print('Starting setup AP (no/bad WiFi config)')
    setup_mode = True
    ...
    try:
        ...
        pass
    ip = ap.ifconfig()[0]
    print('Setup AP active: %s - open http://%s' % (ip, ip))
    return ip, True, True
...
ip, ap_mode, _ = connect_wifi(config.get('SSID', ''), config.get('PASSWORD', ''))
if not ip:
    print('Running offline mode')

if setup_mode:
    current_effect = Marquee(display, 'SETUP %s' % ip, FONT_5X7, mode='scroll')
    effect_name = 'setup'
...
@server.route('/')
def index(params):
    # In setup mode serve the WiFi configuration portal
    if setup_mode:
        return create_config_html()
...
@server.route('/scan')
def scan_wifi(params):
    """Scan nearby WiFi networks (JSON list)."""
    try:
        ...
        for entry in nets:
            ...
            rssi = entry[3]
            auth = entry[4]
            s = ssid.decode() if isinstance(ssid, (bytes, bytearray)) else ssid
            if s and s not in seen:
                seen.add(s)
                result.append({'ssid': s, 'rssi': rssi, 'secure': auth != 0})
...
@server.route('/save_config', methods=('POST',))
def save_config(params):
    """Save WiFi credentials from the setup portal and reboot."""
    ssid = params.get('ssid', '').strip()
    password = params.get('password', '')
    web_password = params.get('web_password', '').strip()
    if not ssid:
        return '<script>alert("SSID required");history.back()</script>'
    try:
        with open('config.txt', 'w') as f:
            f.write('# Generated by setup portal\n')
            f.write('SSID=%s\n' % ssid)
            f.write('PASSWORD=%s\n' % password)
            if web_password:
                f.write('WEB_PASSWORD=%s\n' % web_password)
        print('Config saved for SSID=%s, rebooting' % ssid)
        server.pending_reset = True
        return '<html><body style="font-family:monospace;text-align:center;padding:60px;color:#9098a4"><h2>Saved. Rebooting...</h2></body></html>'
    except Exception as e:
        print('Save config error: %s' % e)
        return '<script>alert("Save failed");history.back()</script>'
...
# WiFi deauth attack monitor (only meaningful when joined to a network)
_wifi_mon = WifiMonitor(_wlan_sta) if (ip and not ap_mode and _wlan_sta) else None
_prev_effect = None
...
def health(params):
    ...
    try:
        ...
        return json.dumps({
            ...
            'ip': ip,
            'ap_mode': ap_mode,
            'defcon': _wifi_mon.defcon if _wifi_mon else 5,
            'defcon_mode': _wifi_mon.mode if _wifi_mon else 'none'
        })
    except Exception as e:
        return '{"error":"%s"}' % str(e)
...
while True:
    try:
        ...
        if time.ticks_diff(now, last_update) >= FRAME_INTERVAL_MS:
            # WiFi attack (deauth) guard: override effect when under attack
            if _wifi_mon:
                lvl = _wifi_mon.update()
                if lvl <= 2 and not isinstance(current_effect, DefconEffect):
                    _prev_effect = current_effect
                    current_effect = DefconEffect(display, FONT_5X7, lvl)
                    _log('DEFCON %d - WiFi attack suspected' % lvl)
                elif isinstance(current_effect, DefconEffect):
                    if lvl <= 2:
                        current_effect.set_level(lvl)
                    else:
